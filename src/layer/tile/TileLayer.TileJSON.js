L.TileLayer.TileJSON = L.TileLayer.Canvas.extend({
    ...
    _drawPoint: function (ctx, coords) {
        var p = this._tilePoint(ctx, coords);
    ...
    _draw: function (ctx) {
        ...
        loader(url, function (data) {
            for (var i = 0; i < data.features.length; i++) { //data.features.length
                ...
                switch (type) {
                    case 'Point':
                        self._drawPoint(ctx, coords);
                        break;

                    case 'MultiPoint':
                        for (j = 0; j < len; j++) {
                            self._drawPoint(ctx, coords[j]);
                        }
                        break;

                    case 'LineString':
                        self._drawLineString(ctx, coords);
                        break;

                    case 'MultiLineString':
                        for (j = 0; j < len; j++) {
                            self._drawLineString(ctx, coords[j]);
                        }
                        break;

                    case 'Polygon':
                        self._drawPolygon(ctx, coords);
                        break;

                    case 'MultiPolygon':
                        for (j = 0; j < len; j++) {
                            self._drawPolygon(ctx, coords[j]);
                        }
                        break;
